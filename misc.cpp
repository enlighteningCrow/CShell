...
template <class T1, class T2>
Pair<T1, T2>::Pair(const std::pair<T1, T2>& list) : first{ list.size() > 0 ? list.begin()[0] : T1{} }, second{ list.size() > 1 ? list.begin()[1] : T2{} } {}
...
template <class T>
bool Vector<T>::operator<(const Vector<T>& vectorA) const {
	return (m_x == vectorA.m_x ? (m_y == vectorA.m_y ? (m_z == vectorA.m_z ? false : m_z < vectorA.m_z) : m_y < vectorA.m_y) : m_x < vectorA.m_x);
	//return ((m_x < vectorA.m_x) || (m_y < vectorA.m_y) || (m_z < vectorA.m_z));
}
...
template <class T>
bool Vector<T>::operator<=(const Vector<T>& vectorA) const {
	return (m_x == vectorA.m_x ? (m_y == vectorA.m_y ? (m_z == vectorA.m_z ? true : m_z < vectorA.m_z) : m_y < vectorA.m_y) : m_x < vectorA.m_x);
	//return ((m_x <= vectorA.m_x) || (m_y <= vectorA.m_y) || (m_z <= vectorA.m_z));
}
...
template <class T>
long int findFirst(T* array, long first, long last, const std::function<short(T*, long)>& determinator) {//short int(*determinator)) {
...
template <>
long int findFirst(Piece* array, long first, long last, const std::function<short(Piece*, long)>& determinator) {//short int(*determinator)) {
	//return (((first + last) / 2) ==  - 1)findFirst()
	if (first == last) { return -1; }
	switch (determinator(array, (first + last) / 2)) {
	case 1: {
...
template <>
long int findFirst(Vector<long int>* array, long first, long last, const std::function<short(Vector<long int>*, long)>& determinator) {//short int(*determinator)) {
	//return (((first + last) / 2) ==  - 1)findFirst()
	if (first == last) { return -1; }
	switch (determinator(array, (first + last) / 2)) {
	case 1: {
...
template <class T>
long int findFirst(const std::vector<T>& array, long first, long last, const std::function<short(const std::vector<T>&, long int)>& determinator) {//short int(*determinator)) {
	//return (((first + last) / 2) ==  - 1)findFirst()
	if (first == last) { return -1; }
	switch (determinator(array, (first + last) / 2)) {
	case 1: {
...
template <>
long int findFirst(const std::vector<Vector<long int>>& array, long first, long last, const std::function<short(const std::vector<Vector<long int>>&, long int)>& determinator) {//short int(*determinator)) {
	//return (((first + last) / 2) ==  - 1)findFirst()
	if (first == last) { return -1; }
	switch (determinator(array, (first + last) / 2)) {
	case 1: {
...
Move::Move(const Vector<long int>& vector, bool recursive) : Pair<Vector<long int>, bool>{ vector, recursive }, vector{ first },
recursive{ second } {}

Move::Move(const Vector<long int>& vector, bool recursive, Move* capture, short captureNext) : Pair<Vector<long int>, bool>{ vector, recursive }, vector{ first }, recursive{ second }, captureNext{ captureNext }, capture{ capture } {}
