...
public:
    ...
    public:
        BitsetLookup(T& val, std::size_t m_offset) : m_val(val), m_offset(m_offset) {
        }
        operator bool() {
            return m_val & T(1) << m_offset;
        }
    ...
    Bitset(std::size_t size, bool default_val) : m_size(size), m_data(ceilDiv(ceilDiv(size, BPB), sizeof(T))) {
        ...
        if (default_val) {
            ...
            for (std::size_t i{0}; i < awayCeil; ++i) {
                m_data[until] |= (T(1) << i);
            }
        }
        else {
        ...
        if (val) {
            if (awayCeilB == awayCeilE) {
                for (size_t i(inB); i < inE; ++i) {
                    m_data[awayCeilB] |= (T(1) << i);
                }
                return *this;
            }
            for (size_t i(inB); i < sizeof(T); ++i) {
                m_data[awayCeilB] |= (T(1) << i);
            }
            for (size_t i(awayCeilB); i <= awayCeilE; ++i) {
                m_data[i] |= ~T(0);
            }
            for (size_t i(0); i < inE; ++i) {
                m_data[awayCeilE] |= (T(1) << i);
            }
            // if (awayCeilB) {
    ...
    friend std::ostream& operator<<(std::ostream& out, const Bitset<T>& bs) {
        for (std::size_t i{0}; i < bs.m_size; ++i) {
            out << bs(i);
        }
        return out;
    }
