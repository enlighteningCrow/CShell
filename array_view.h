...
public:
    ...
    Array_view<T> &setViewStart() { m_start = 0; }
    Array_view<T> &setViewEnd(std::size_t end) { m_end = end; }
    Array_view<T> &setViewEnd() { m_end = arr.size(); }
    ...
    Array_view<T> &truncFrontIgnore(std::size_t size, const T &ignore) {
        ...
        for (std::size_t i{m_start}; i < size() && counter <= size; ++i) counter += ignore != (*this)[i];
...
public:
    ...
    bool match_starting(const String &target) {
        if (target.size() < size()) return false;
        return (memcmp(&(operator[](0)), &(target[0]), size()));
    }
