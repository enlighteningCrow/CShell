...
public:
    Array_view(const Array<T>& arr) : m_arr{ arr }, m_start{ 0 }, m_end{ arr.size() } {}
    Array_view(const Array_view<T>& arr) : m_arr{ arr.m_arr }, m_start{ arr.m_start }, m_end{ arr.m_end /* arr.size() - 1 */ } {}
    std::size_t size() { return m_end - m_start; }
    std::size_t start() { return m_start; }
    std::size_t end() { return m_end; }
    ...
    Array_view<T>& truncFront(std::size_t size) { setViewStart(m_start + size); return *this; }
    Array_view<T>& truncBack(std::size_t size) { setViewEnd(m_end - size); return *this; }
    Array_view<T>& extendFront(std::size_t size) {
        if (size <= m_start) setViewStart(m_start - size);
        else *stderr << "Out of range";
        return *this;
    }
    Array_view<T>& extendBack(std::size_t size) {
        if ((size + m_end) <= m_arr.size()) setViewEnd(m_end + size);
        else *stderr << "Out of range";
        return *this;
    }
...
public:
    ...
    std::size_t findStartingWith(const T1& key) {
