...
public:
    ...
    Array<T>& resize(size_ut size) {
        ...
        if (m_size == size)
            return *this;
        if ((size <= m_allocated) && (size * 0.7 >= m_allocated)) {
            goto afterRealloc;
        }
    ...
    bool operator==(const Array<T>& arr) {
        if (arr.size() != size())
            return false;
        return memcmp(m_data, arr.m_data, size());
    }
    ...
    bool operator<(const Array<T>& arr) {
        ...
        return memcmp(m_data, arr.m_data, size()) < 0;
    }
...
public:
    ...
    inline bool operator<(const Array<T>& arr) const {
        ...
        return (memcmp(m_data, arr.m_data, m_size)) < 0;
    }
    inline bool operator>(const Array<T>& arr) const {
        if (m_size != arr.m_size) {
            if (m_size > arr.m_size)
...
public:
    ...
    // String(String&& array) : Array<char>{array} {
    // }
    String(const String& array) : Array<char>{array} {
    }
    String(size_ut size) : Array<char>(size) {
    }
    String(const char* str) : Array<char>((size_ut)strlen(str)) {
        memcpy(m_data, str, m_size);
    }
    String(String_view& strview) : Array<char>(strview.size());
    ...
    String operator+(const char* array) const {
        String arr{m_size + (size_ut)(strlen(array))};
        memcpy(arr.m_data, m_data, m_size);
    ...
    friend String operator+(const char* array, const String& str) {
        // TODO: this
        String arr{str.m_size + (size_ut)(strlen(array))};
...
public:
    ...
    Array<T1> keys() const {
        ...
        for (size_ut i{0}; i < this->m_size; ++i) {
            keyArr[i] = this->m_data[i].first;
        }
        return keyArr;
    }
    ...
    virtual size_ut find(const T1& target) {
        ...
        return _find(target, 0, this->m_size - 1);
    }
    virtual size_ut findGreater(const T1& target) {
        ...
        return _findg(target, 0, this->m_size - 1);
    }
    virtual size_ut findGreaterEq(const T1& target) {
        ...
        return _findge(target, 0, this->m_size - 1);
    }
    virtual size_ut findLesser(const T1& target) {
        ...
        return _findl(target, 0, this->m_size - 1);
    }
    virtual size_ut findLesserEq(const T1& target) {
        ...
        return _findle(target, 0, this->m_size - 1);
    }
    ...
    virtual size_ut _find(const T1& target, size_ut left, size_ut right) {
        if (right < left)
            return this->m_size;
        this->_middle = (left + (right - left) / 2);
        if (this->m_data[this->_middle].first == target)
            return this->_middle;
        else if (this->m_data[this->_middle].first > target)
            return _find(target, left, this->_middle - 1);
        else
            return _find(target, this->_middle + 1, right);
    }
    virtual size_ut _findg(const T1& target, size_ut left, size_ut right) {
        ...
        this->_middle = (left + (right - left) / 2);
        if (this->m_data[this->_middle].first == target)
            return _findg(target, this->_middle + 1, right);
        else if (this->m_data[this->_middle].first > target) {
            if (this->_middle == 0) {
                return this->_middle;
            }
            if (!(this->m_data[this->_middle - 1].first > target))
                return this->_middle;
            return _findg(target, left, this->_middle - 1);
        }
        else
            return _findg(target, this->_middle + 1, right);
    }
    virtual size_ut _findl(const T1& target, size_ut left, size_ut right) {
        ...
        this->_middle = (left + (right - left) / 2);
        if (this->m_data[this->_middle].first == target)
            return _findl(target, this->_middle + 1, right);
        else if (this->m_data[this->_middle].first > target) {
            return _findl(target, left, this->_middle - 1);
        }
        else {
            if (this->_middle == this->m_size - 1) {
                return this->_middle;
            }
            if (!(this->m_data[this->_middle + 1].first < target))
                return this->_middle;
    ...
    virtual size_ut _findge(const T1& target, size_ut left, size_ut right) {
        ...
        if (this->m_data[this->_middle].first == target) {
            ...
            return _findge(target, this->_middle + 1, right);
        }
        else if (this->m_data[this->_middle].first > target) {
            if (this->_middle == 0) {
                return this->_middle;
            ...
            if (!(this->m_data[this->_middle - 1].first > target))
                return this->_middle - 1;
            return _findge(target, left, this->_middle - 1);
        }
        else
            return _findge(target, this->_middle + 1, right);
    }
    virtual size_ut _findle(const T1& target, size_ut left, size_ut right) {
        ...
        if (this->m_data[this->_middle].first == target) {
            ...
            return _findle(target, this->_middle + 1, right);
        }
        else if (this->m_data[this->_middle].first > target) {
            return _findle(target, left, this->_middle - 1);
        }
        else {
            if (this->_middle == this->m_size - 1) {
                return this->_middle;
