...
public:
    ...
    Types(const String& name, const Array<Pair<Types, String>>& vars) : type_name{ name }, type_vars{ vars } {
...
Array<Types> types{
        Types{"char", Array<Pair<Types, String>>(0) },
...
public:
    ...
    bool operator==(const Types_bck& type) const {
            ...
            for (int i{ 0 }; i < (*iter).second.size(); ++i, counter = (counter && !paren) ? counter - 1 : counter) {
                ...
                case 'o':
                    if (is_first) {
                        ...
                        case 'e':
                            ep = Expectance{ Statement::Types::OPERATOR | Statement::Modes::PREFIX };
                            break;
                        case 'o':
                            ep = Expectance{ Statement::Types::OPERATOR | Statement::Modes::POSTFIX };
                            break;
                        case 'b':
                            ep = Expectance{ Statement::Types::OPERATOR | Statement::Modes::BOTH };
                            break;
 ...
 *          Pair of:
 *              evaluated type (if -1, means dependent on type of function; if -2, means dependent on type of variable)
 *              id of statement type
 *  Note: STypes stands for Statement Types
 *  Upon seing anything with return value with higher level of precedence, evaluate it first.
 *  Note: Support for default-int
 */
    ...
    Pair<Array<std::size_t>, Pair<std::size_t, std::size_t>>{
        {
            ...
            STypes::COMMA | STypes::TYPE | STypes::NAME | STypes::MULTIPLE | STypes::OPTIONAL,            // STypes::COMMA_TYPE_AND_NAME | STypes::OPTIONAL | STypes::MULTIPLE,
                ...
                Pair<Array<std::size_t>, Pair<std::size_t, std::size_t>>{
                    {
                        ...
                        STypes::COMMA | STypes::NAME | STypes::LITERAL | STypes::MULTIPLE | STypes::OPTIONAL,            // STypes::COMMA_TYPE_AND_NAME | STypes::OPTIONAL | STypes::MULTIPLE,
                        STypes::PARENTHESIS | STypes::CLOSE,
                    },
...
//     std::pair<std::string, std::vector<Expectance>> {"auto", {Expectance{Statement::Types::TYPE_MODIFIER}}},
//     std::pair<std::string, std::vector<Expectance>> {"break", {Expectance{Statement::Types::STATEMENT}}},
//     std::pair<std::string, std::vector<Expectance>> {"case", "lxi_switch_"},
//     std::pair<std::string, std::vector<Expectance>> {"char", "t"},
//     std::pair<std::string, std::vector<Expectance>> {"const", "x(bt|at|it)"},
//     std::pair<std::string, std::vector<Expectance>> {"continue", "sxi(_for_|_while_|_do_)"},
//     std::pair<std::string, std::vector<Expectance>> {"default", "lxi_switch_"},
//     std::pair<std::string, std::vector<Expectance>> {"do", "xbb"},
//     std::pair<std::string, std::vector<Expectance>> {"double", "t"},
...
//     std::pair<std::string, std::vector<Expectance>> {"sizeof", "vc(t|n@T0#)"},
//     std::pair<std::string, std::vector<Expectance>> {"static", "xbd(v)"},
//     std::pair<std::string, std::vector<Expectance>> {"struct", "z"},
//     std::pair<std::string, std::vector<Expectance>> {"switch", "xbb"},
//     std::pair<std::string, std::vector<Expectance>> {"typedef", "xbtn"},
//     std::pair<std::string, std::vector<Expectance>> {"union", "z"},
//     std::pair<std::string, std::vector<Expectance>> {"unsigned", "xbt"},
//     std::pair<std::string, std::vector<Expectance>> {"void", "t"},
//     std::pair<std::string, std::vector<Expectance>> {"volatile", ""},
//     std::pair<std::string, std::vector<Expectance>> {"while", "c(n@b#)b|xbb"},
//     std::pair<std::string, std::vector<Expectance>> {"#define", "r(1/2)"},
//     std::pair<std::string, std::vector<Expectance>> {"#elif", "rv(b)"},
//     std::pair<std::string, std::vector<Expectance>> {"#else", "r0"},
//     std::pair<std::string, std::vector<Expectance>> {"#endif", "r0"},
//     std::pair<std::string, std::vector<Expectance>> {"#error", "rv(c*)"},
//     std::pair<std::string, std::vector<Expectance>> {"#if", "rv(b)"},
//     std::pair<std::string, std::vector<Expectance>> {"#ifdef", "rn"},
//     std::pair<std::string, std::vector<Expectance>> {"#ifndef", "rn"},
//     std::pair<std::string, std::vector<Expectance>> {"#include", "r_<_n_>_|r_\"_n_\"_"},
 ...
 *  Note: if -1 or -2, check for type being modified, and check below for how many possible:
 *      if -1 check index 0:
 *          if type is (id for int), 2 allowed, if type is (id for double), 1 allowed, if other, 0 allowed
 *      if -2 check index 1:
 *          if type is (id for int), 2 allowed, if type is (id for double), 1 allowed, if other, 0 allowed
...
Array <Pair<Pair<std::size_t, std::size_t>, JunctionMode>> location_specifics{
    Pair<Pair<std::size_t, std::size_t>, JunctionMode>(Pair<std::size_t, std::size_t>(STypes::ELSE, STypes::IF), JunctionMode::AFTER),
    Pair<Pair<std::size_t, std::size_t>, JunctionMode>(Pair<std::size_t, std::size_t>(STypes::CASE, STypes::SWITCH), JunctionMode::IN),
    Pair<Pair<std::size_t, std::size_t>, JunctionMode>(Pair<std::size_t, std::size_t>(STypes::BREAK, STypes::SWITCH), JunctionMode::IN),
};
// Array<Pair
...
std::map<std::string, std::size_t> key_expressions{
    ...
    std::pair<std::string, std::size_t> {"#include", STypes::PREPROCESSOR_INCLUDE},
    ...
    std::pair<std::string, std::size_t> {"+", STypes::OPERATOR | STypes::BOTH },
    std::pair<std::string, std::size_t> {"-", STypes::OPERATOR | STypes::BOTH | STypes::PREFIX },
    std::pair<std::string, std::size_t> {"*", STypes::OPERATOR | STypes::BOTH },
    std::pair<std::string, std::size_t> {"/", STypes::OPERATOR | STypes::BOTH },
    std::pair<std::string, std::size_t> {"%", STypes::OPERATOR | STypes::BOTH },
    std::pair<std::string, std::size_t> {"+=", STypes::OPERATOR | STypes::BOTH | STypes::REQUIRE_LVALUE },
    std::pair<std::string, std::size_t> {"-=", STypes::OPERATOR | STypes::BOTH | STypes::REQUIRE_LVALUE },
    std::pair<std::string, std::size_t> {"*=", STypes::OPERATOR | STypes::BOTH | STypes::REQUIRE_LVALUE },
    std::pair<std::string, std::size_t> {"/=", STypes::OPERATOR | STypes::BOTH | STypes::REQUIRE_LVALUE },
    std::pair<std::string, std::size_t> {"%=", STypes::OPERATOR | STypes::BOTH | STypes::REQUIRE_LVALUE },
    ...
    std::pair<std::string, std::size_t> {"^=", STypes::OPERATOR | STypes::BOTH | STypes::REQUIRE_LVALUE },
    std::pair<std::string, std::size_t> {"|=", STypes::OPERATOR | STypes::BOTH | STypes::REQUIRE_LVALUE },
    std::pair<std::string, std::size_t> {"++", STypes::OPERATOR | STypes::PREFIX | STypes::POSTFIX},
    std::pair<std::string, std::size_t> {"--", STypes::OPERATOR | STypes::PREFIX | STypes::POSTFIX},
    std::pair<std::string, std::size_t> {"==", STypes::OPERATOR | STypes::BOTH},
    std::pair<std::string, std::size_t> {"!=", STypes::OPERATOR | STypes::BOTH},
    std::pair<std::string, std::size_t> {">", STypes::OPERATOR | STypes::BOTH},
    ...
    std::pair<std::string, std::size_t> {"&&", STypes::OPERATOR | STypes::BOTH},
    std::pair<std::string, std::size_t> {"||", STypes::OPERATOR | STypes::BOTH},
    std::pair<std::string, std::size_t> {"?", STypes::OPERATOR | STypes::QUESTION},
