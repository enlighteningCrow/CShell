...
public:
    ...
    public:
        ...
        std::shared_ptr<Expression> & indexAsExp(size_ut index) const { return (subs[index].first).ex; }
        Pair<String, ExpressionType> &indexAsStr(size_ut index) const { return (subs[index].first).str; }
                    ...
                    for (holder = 0; holder < 15; ++holder) {
                        ...
                        // TODO!: Make an Array_view of whatever to add with (the things that are about to get replaced)
                        {
                            ...
                            for (size_ut j{i}; j < i + increment; ++j) {
                                ...
                                if ((index != mapb.size())) {
                                    ...
                                    if (arr[-1].second && arr[-2].second) {
                                        size_ut a{0}, b{0};
                                        for (char t{-1}; t >= -2; --t) {
                                            for (size_ut i{0}; i < arr[t].first.ex->subs.size(); ++i) {
                                                if (arr[t].first.ex->subs[i].second) continue;
                                                // if (arr[t].first.str.second[i].first | STypes::OPERATOR) {
                                                if (arr[t].first.ex->subs[i].first | STypes::OPERATOR) {
                                                    (t == -1 ? a : b) = arr[t].first.str.second[i].first;
                                                    break;
                                                }
                                            }
                                        }
                                        if (b == a) { switch () }
                                        // TODO: Check if the two contain operators with same precedence, if yes, check
                                        // if needed right-to-left instead, if so, swap their overarching ones.(one that
                                        // is matching operators (not necessarily the same one.))
                                    }
                                    ...
                                    arr.push_back(Pair<Expression::ExpressionStr, bool>(
                                        Expression::ExpressionStr(
                                            Pair<String, ExpressionType>(arr_view[j].first, arr_view[j].second)),
                                        0));
                                }
                            }
                            ...
                            for (std::size_t cnt{i}; cnt < i + increment;) {
                                if ((found_index = mapb.findGreaterEq(cnt)) < i + increment) {
                        ...
                        if (!holder) {
                            ...
                            for (size_ut k{0}; k < statement_types_arrays[i].first.size(); ++k) {
                                for (size_ut l{0}; l < statement_types_arrays[i].first[k].first.size(); ++l) {
                                    if (statement_types_arrays[i].first[k].first[l] & STypes::OPERATOR) { ++sum; }
                                }
                            }
                            if (!sum) break;
